...
template <typename T>
using EnumInfo = decltype(craystalFindEnumInfoADL(std::declval<T>()));
...
#define CRAYSTAL_ENUM_REGISTER(T)                                              \
    constexpr T##_info craystalFindEnumInfoADL [[maybe_unused]] (T) noexcept { \
        return T##_info{};                                                     \
    }

/// Enum formatter.
template <typename T>
struct fmt::formatter<T, std::enable_if_t<CRay::has_enum_info_v<T>, char>>
    : formatter<std::string> {
    template <typename FormatContext>
    auto format(const T& e, FormatContext& ctx) {
        return formatter<std::string>::format(CRay::enumToString(e), ctx);
    }
};
